...
const searchData = [
    ...
    { id: 'artikel1.html', name: 'Review: White Goddess', brand: 'News', type: 'news' },
...
function showToast(message) {
    ...
    toast.className = `
        ...
        text-[10px] font-bold uppercase tracking-widest shadow-2xl 
        z-[9999] transition-all duration-500 opacity-0 translate-y-5
        whitespace-nowrap
    `;
...
// 3. FUNGSI NAVIGASI UTAMA (Optimasi Mobile Redirect)
function processNavigation(itemName) {
    const item = searchData.find(s => s.name.toLowerCase() === itemName.toLowerCase());
    
    if (item && (item.type === 'news' || item.type === 'page')) {
        window.location.assign(item.id);
    } else if (item) {
        localStorage.setItem('scrollToPerfume', item.name.toLowerCase());
...
// 4. FUNGSI AUTOCOMPLETE (Touch Friendly)
function showSuggestions() {
    ...
    if (!box) {
        ...
        box.className = 'absolute top-full left-0 w-full bg-white shadow-xl rounded-b-md z-[2000] border border-gray-200';
    ...
    if (matches.length > 0) {
        box.innerHTML = matches.map(item => `
            <div onclick="processNavigation('${item.name}')" 
                 class="px-5 py-3 border-b active:bg-gray-200 hover:bg-gray-100 cursor-pointer flex justify-between items-center text-black">
                <div class="pointer-events-none">
                    <span class="text-sm font-bold text-gray-800">${item.name}</span>
                    <p class="text-[10px] text-gray-400 uppercase">${item.brand}</p>
                </div>
                <span class="text-[9px] font-black px-2 py-1 bg-gray-100 rounded text-gray-500 uppercase pointer-events-none">${item.type}</span>
            </div>
        `).join('');
    } else {
...
// 6. FUNGSI AUTO-SCROLL (Optimasi Delay Mobile)
function executeAutoScroll() {
    const targetName = localStorage.getItem('scrollToPerfume');
    if (!targetName) return;

    // Gunakan interval kecil untuk memastikan element sudah ter-render di mobile
    let attempts = 0;
...
function loadHomeContent() {
    ...
    if (newGrid) {
        newGrid.innerHTML = homeData.newArrivals.map(p => `
            <div class="perfume-card-mini cursor-pointer" onclick="processNavigation('${p.name}')">
                <div class="overflow-hidden rounded-md mb-2">
                    <img src="${p.img}" class="w-full h-auto shadow-sm">
                </div>
    ...
    if (bestList) {
        bestList.innerHTML = homeData.bestSellers.map((p, index) => `
            <div class="best-item active:bg-gray-100 cursor-pointer flex items-center p-3 gap-4" onclick="processNavigation('${p.name}')">
...
// 8. INISIALISASI (Mobile Friendly Events)
document.addEventListener('DOMContentLoaded', () => {
    const input = document.getElementById('mainSearch');
    if (input) {
        input.addEventListener('input', showSuggestions);
        // Tangani tombol "Go" atau "Search" di keyboard HP
        input.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                performSearch();
                input.blur(); // Tutup keyboard HP setelah search
            }
        });
    }

    // Klik di luar untuk tutup dropdown (Support Touch)
    document.addEventListener('touchstart', (e) => {
        const box = document.getElementById('searchSuggestions');
        if (box && e.target.id !== 'mainSearch' && !box.contains(e.target)) {
